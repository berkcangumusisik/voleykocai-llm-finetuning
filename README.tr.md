...
# 2) Elle yazılmış 20 tohumu çoğalt          [ANTHROPIC_API_KEY varsa model modu]
...
| `synthetic` | 60 | Elle yazdığım 20 tohumdan çoğaltma |
...
`augment.py` bunları çoğaltıyor. İki modu var: `ANTHROPIC_API_KEY` varsa modele veriyor ve gerçekten yeni soru-cevap çiftleri üretiyor; yoksa soruyu şablonlarla yeniden yazıp cevabı tohumdan kopyalıyor.

> **Bu depodaki `train.jsonl` çevrimdışı modda üretildi.** Yani sentetik kısımdaki 60 örneğin cevapları 20 tohumdan kopya. `reports/dataset_stats.md` bunu benzersiz cevap oranıyla raporluyor (166'da 125, %75). Bir API anahtarıyla `augment.py`'yi tekrar çalıştırırsan hem oran hem toplam örnek sayısı ciddi şekilde yükselir.
